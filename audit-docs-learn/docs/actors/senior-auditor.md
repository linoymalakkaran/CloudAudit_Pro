---
sidebar_position: 5
title: Senior Auditor Role
description: Expert practitioner handling complex procedures and mentoring junior staff
---
...
```mermaid
graph TD
    A[New Topic: Stock Options] --> B[Review Standards]
    B --> C[Consult Resources]
    C --> D[Ask Experts]
    D --> E[Document Approach]
    
    B --> F[ASC 718<br/>Stock Compensation]
    C --> G[Firm Guide<br/>Tech CPE Course]
    D --> H[David<br/>Technical Team]
...
```mermaid
graph TD
    A[Test Sample of 30 Contracts] --> B{Errors Found?}
    B -->|Yes| C[Document Each Error]
    B -->|No| D[Document as Correct]
...
```mermaid
graph TD
    A[Open Workpaper] --> B[Review Objective]
    B --> C[Check Work Performed]
    C --> D{Quality OK?}
    D -->|Yes| E[Check Documentation]
    D -->|Issues| F[Note Issues]
    E --> G{Documentation OK?}
    G -->|Yes| H[Check Conclusion]
    G -->|Issues| F
    H --> I{Conclusion OK?}
    I -->|Yes| J[Approve]
    I -->|Issues| F
    F --> K[Return to Emma with Comments]
...
```mermaid
graph LR
    A[Staff Auditor<br/>2-3 years] --> B[Senior Auditor<br/>2-4 years]
    B --> C[Manager<br/>2-4 years]
    C --> D[Partner<br/>Career]
