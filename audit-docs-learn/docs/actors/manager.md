...
sidebar_position: 4
title: Manager Role
description: Team leader overseeing audit engagements from planning to completion
---
...
```mermaid
sequenceDiagram
    participant M as Mike (Manager)
    participant S as Sarah (Senior)
    participant E as Emma (Staff)
    
    M->>S: Explain audit approach
    M->>E: Explain audit approach
    M->>S: Assign revenue procedures (15)
    M->>E: Assign cash procedures (8)
    S->>M: Questions about SaaS revenue
    M->>S: Will discuss with client first
    E->>M: When do we start fieldwork?
    M->>E: Feb 1, after trial balance received
```
...
```mermaid
graph LR
    A[Receive TB] --> B[Review for Completeness]
    B --> C[Import to CloudAudit]
    C --> D[Verify Balance]
    D --> E[Assign Procedures]
    E --> F[Notify Team]
...
```mermaid
graph TD
    A[Issue Identified] --> B[Assess Materiality]
    B --> C{Material?}
    C -->|Yes| D[Discuss with Client]
    C -->|No| E[Document & Monitor]
    D --> F[Client Agrees?]
    F -->|Yes| G[Propose Adjustment]
    F -->|No| H[Further Discussion]
    G --> I[Document Resolution]
...
```mermaid
graph TD
    A[Emma Completes Work] --> B[Mike Reviews]
    B --> C{Quality OK?}
    C -->|Yes| D[Approve]
    C -->|Issues| E[Return with Comments]
    E --> A
    
    F[Sarah Completes Work] --> G[Mike Reviews]
    G --> H{Quality OK?}
    H -->|Yes| I[Approve]
    H -->|Issues| J[Return with Comments]
    J --> F
    
    D --> K[All Work Approved]
    I --> K
    K --> L[Final Audit Review]
...
```mermaid
graph TD
    A[Notice Pattern] --> B[Gather Facts]
    B --> C[Private Conversation]
    C --> D{Root Cause?}
    D -->|Lack of Skills| E[Provide Training]
    D -->|Too Much Work| F[Redistribute Load]
    D -->|Personal Issues| G[Offer Support]
    D -->|Attitude| H[Performance Plan]
    
    E --> I[Monitor Improvement]
    F --> I
    G --> I
    H --> I
...
```mermaid
graph LR
    A[Senior Auditor] --> B[Manager]
    B --> C[Senior Manager]
    C --> D[Partner]
    
    B --> E[Typical: 2-4 years]
